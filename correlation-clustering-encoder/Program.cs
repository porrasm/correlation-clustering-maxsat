...
Parser.Default.ParseArguments<Args>(args).WithParsed(parsed => {
    ...
    Benchmarks.BenchmarkEncodings(problem, ICrlClusteringEncodingBase.GetEncodings(new MinimumRangeMultiplied(), parsed.Encodings == null ? null : parsed.Encodings.ToArray()));
}).WithNotParsed(parse => {
